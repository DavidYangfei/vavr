...
class TryTest {
    ...
    void shouldConsumeSuccessWithForEach() {
        ...
        assertEquals(list, List.of(SUCCESS_VALUE));
    }

    @Test
    void shouldThrowNPEWhenConsumingFailureWithForEachAndActionIsNull() {
        //noinspection ConstantConditions
        assertThrows(NullPointerException.class, () -> FAILURE.forEach(null)).getMessage();
    }

    @Test
    void shouldThrowNPEWhenConsumingSuccessWithForEachAndActionIsNull() {
        //noinspection ConstantConditions
        assertThrows(NullPointerException.class, () -> SUCCESS.forEach(null)).getMessage();
    }
    ...
    void shouldThrowNullCauseWrappedInRuntimeExceptionWhenGetOnFailure() {
        assertSame(
                assertThrows(NonFatalException.class, () -> Try.failure(null).get()).getCause(),
                null
        );
    }
    ...
    void shouldCaptureExceptionWhenRecoverWithFailure() {
        assertEquals(FAILURE.recoverWith(Throwable.class, ignored -> { throw ERROR; }), Try.failure(ERROR));
    }
    ...
    void shouldStreamFailure() {
        assertEquals(FAILURE.stream().collect(Collectors.toList()), Collections.emptyList());
    }

    @Test
    void shouldStreamSuccess() {
        assertEquals(SUCCESS.stream().collect(Collectors.toList()), Collections.singletonList(SUCCESS_VALUE));
    }
    ...
    void shouldConvertFailureToEitherUsingIdentityThrowableMapper() {
        assertEquals(FAILURE.toEither(Function.identity()), Either.left(FAILURE_CAUSE));
    }

    @Test
    void shouldConvertFailureToEitherUsingNonTrivialThrowableMapper() {
        assertEquals(FAILURE.toEither(ignored -> ERROR), Either.left(ERROR));
    }

    @Test
    void shouldConvertSuccessToEitherUsingIdentityThrowableMapper() {
        assertEquals(SUCCESS.toEither(Function.identity()), Either.right(SUCCESS_VALUE));
    }
    ...
    void shouldConvertFailureToOption() {
        assertEquals(FAILURE.toOption(), Option.none());
    }


    @Test
    void shouldConvertSuccessOfNonNullToOption() {
        assertEquals(SUCCESS.toOption(), Option.some(SUCCESS_VALUE));
    }

    @Test
    void shouldConvertSuccessOfNullToOption() {
        assertEquals(Try.success(null).toOption(), Option.some(null));
    }
    ...
    void shouldSerializeFailure() throws IOException, ClassNotFoundException {
        ...
        assertSame(testee.getCause().getClass(), FAILURE.getCause().getClass());
        assertEquals(testee.getCause().getMessage(), FAILURE.getCause().getMessage());
    }

    @Test
    void shouldSerializeSuccess() throws IOException, ClassNotFoundException {
        assertEquals(deserialize(serialize(SUCCESS)), SUCCESS);
    }
